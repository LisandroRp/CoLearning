...
class Mapa extends Component {
  ...
  obtenerPos(okPos) {
    this.setState({ longitude: UserDataManager.getInstance().getLongitude(), latitude: UserDataManager.getInstance().getLatitude() })
    okPos()
  }
  okPos() {
  ...
  comprobar(marker) {
    for (var i = 0; i < this.state.cambiados.length; i++) {
  ...
  createMarker(marker) {
    var cantidad = 0;
    for (var i = 0; i < this.state.coordenadasGuardadas.length; i++) {
          ...
          <View style={styles.container}>
            ...
            */}
            <SearchBar
              ...
              inputContainerStyle={{ backgroundColor: '#A2C8FF', paddingTop: 0 }}
              placeholderTextColor='#A01A50'
              containerStyle={{ backgroundColor: '#6BA8FF', paddingTop: 0 }}
              buttonStyle={{}}
              searchIcon={{ color: '#A01A50' }}
            />
            <MapView style={{ backgroundColor: this.state.searchBarFocused ? 'rgba(0,0,0,0.3)' : 'red', position: 'absolute', top: 60, left: 0, bottom: 0, right: 0 }}
