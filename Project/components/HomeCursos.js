         ...
         <View style={{ position: 'absolute', bottom: hp(5), left: 0, right: 0, justifyContent: 'center', alignItems: 'center' }}>
          ...
          >
            ...
            {this.state.activeImage ? (this.state.activeImage.id != 0 ? <Text style={styles.domicilioProfesor}>{this.state.activeImage.domicilio}  </Text> : <View></View>) : <View></View>}
              {this.state.activeImage ? ( this.state.activeImage.id == 0 ? <Text style={{textAlign: 'justify', fontSize: wp(4.4)}}>CoLearning te recomienda una gran variedad de cursos a partir de las distintas clases que notamos de tu interés. {'\n'}{'\n'} Además te da la posibilidad de visualizar cuales son los cursos más populares de la aplicación y los cercanos a tu zona actual.</Text> : <Text></Text>): <Text></Text>}
            ...
            {/* </View> */}
              <TouchableOpacity style={[styles.button, { backgroundColor: this.inactiveImageButton() }]} onPress={() => this.props.onPressGo(this.state.activeImage.id, this.state.activeImage.nombre, this.state.activeImage.domicilio)} >
